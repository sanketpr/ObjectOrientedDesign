...
class AbsTree_Iterator {
	...
	private void stack_left_spine(AbsTree node) throws Exception {
		...
		if (node != null) {
			...
			stackMinValue = stack.peek().value;
		}

		AbsTree node2 = node; // use node2 instead of node

		Contract.Ensures(node2 == null || (stack.peek().value == node2.min().value && minNode(stack) == stack.peek().value));
	}
	
	private Integer minNode(Stack<AbsTree> stack) {
		Integer i = null;
		while (!stack.empty()) {
			Integer tmp = stack.pop().value;
			if (i == null || i > tmp) {
				i = tmp;
			}
		}
